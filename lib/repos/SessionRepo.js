...
class SessionRepo {
	...
	*/
	async updateSessionActivity(userId, hashedExpireDate, sessionDetails = {}, prolongExpiryMs = 0) {
		const session = await this._collection.findOne({ id: hashedExpireDate });

		sessionDetails = { ...sessionDetails, lastActivity: new Date() };

		if (session) {
			sessionDetails = { ...sessionDetails, ...session.sessionDetails };
			session.expires = prolongExpiryMs ? new Date(Date.now() + prolongExpiryMs) : session.expires;
		}

		return await this._collection.updateOne({ userId, id: hashedExpireDate }, { $set: { sessionDetails, expires: session.expires } });
	}
