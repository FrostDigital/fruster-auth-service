...
class JWTManager {
	...
	*/
	 ...
	 */
	async encode(user, expiresInMs, sessionDetails) {
		try {
			...
			await this._sessionRepo.add(this._hashExpireDate(expireDate, user.id, salt), user.id, expiresInMs, sessionDetails);

			const jwtObject = Object.assign({
				exp: expireDate, salt
			}, user);

			log.debug("session added");

			return jwt.encode(jwtObject, conf.secret);
		} catch (err) {
			log.error(err);
			throw err;
		}
	...
	async removeOneForUserById(id, userId) {
		try {
			await this._sessionRepo.removeOne(id, userId);
		} catch (err) {
			log.error(err);
			throw err;
		}
	...
	async updateSessionActivity(decodedToken) {
		const { exp, id: userId, salt } = decodedToken;
		const hashedExpireDate = this._hashExpireDate(exp, userId, salt);

		await this._sessionRepo.updateSessionActivity(userId, hashedExpireDate);
	}
