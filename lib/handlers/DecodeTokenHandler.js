...
const conf = require("fruster-log/conf");
...
class DecodeTokenHandler {
	...
	async handle({ reqId, data,
		...
		const { id, exp, salt, asUser, ...rest } = decodedToken;

		const fetchImpersonatedUser = asUser && conf.enableImpersonation;

		if (asUser && !conf.impersonateScope) {
			log.warn(`User ${id} attempted to do impersonation but impersonation is not enabled`);
		}

		// Get fresh user object from user service
		let user = await UserManager.getUser(reqId, id);

		if (!user)
			throw errors.invalidAccessToken(`User with id ${id} does not exist`);

		if (fetchImpersonatedUser) {
			if (!user.scopes.includes(conf.impersonateScope))
				throw errors.impersonationNotAllowed();

			const impersonatedUser = await UserManager.getUser(reqId, id);

			if (!impersonatedUser)
				throw errors.invalidAccessToken(`Cannot impersonate: User with id ${asUser} does not exist`);

			log.info(`User ${id} impersonates ${asUser}`);
