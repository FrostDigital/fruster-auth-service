...
class DecodeTokenHandler {
	...
	async handle({ reqId, data,
		...
		const prolongExpiry = conf.prolongCookieSessionOnActivity && this._isCookieRequest(headers) ? ms(conf.cookieSessionTTL || conf.jwtCookieAge) : 0
		...
		/*
		  ...
		  */
		this._jwtManager.updateSessionActivity(decodedToken, sessionDetails, prolongExpiry).catch(err => log.warn(err));
